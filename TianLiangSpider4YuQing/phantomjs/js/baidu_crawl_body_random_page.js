...
try{
   ...
   //遇到错误后直接退出
	console.log(err.message+'json file parser error,will jump the task,phantom will exit!');
	phantom.exit();
}
...
function openPage(root_url) {
	...
	//如果打开页面成功
		if (status === "success") {
		//如果是某此需要sso验证的页面,暂时只考虑weibo类
			if ((page.url.indexOf("passport.weibo.com")) == -1){
			   if (is_capture_pic) {
                    page.render(random_url_output_path_capture + pic_file_suffix_name);
			   }
			   if (is_data_write_to_file) {
				   fs.write(random_url_output_path_body + data_file_suffix_name,
						page.content, 'w');
			   }
			   exit();   
			}else{
