...
page.onLoadFinished = function(status) {
	 ...
	 * fs.write('page.txt','\n\n********','a');
	 */
	if ((page.url.indexOf("http://news.baidu.com/ns?")) > -1) {
		if (is_capture_pic) {
		...
		page.evaluate(function(current_grab_page_number_copy) {
			...
			var arr = document.querySelectorAll('p#page>a.n');
	...
	} else if (page.url == ("http://news.baidu.com/")) {
		...
		page.evaluate(function(keyword) {
			if (document.getElementById("ww")) {
				...
				document.querySelector('input#ww').value = keyword;
			} else {
				// window.alert("kw1 not exist!");
				document.querySelector('input#ww').value = keyword;
			}

			if (document.getElementsByName("fbaidu")) {
				document.querySelector('form[name="fbaidu"]').submit();
			} else {
				document.querySelector('form[name="fbaidu"]').submit();
			}
