...
page.onLoadFinished = function(status) {
	...
	if ((page.url.indexOf("http://news.haosou.com/ns?")) > -1) {
		if (is_capture_pic) {
			// page.render(pic_capture_save_root_path +
		...
		if (is_data_write_to_file) {
			// fs.write(data_write_to_file_root_path +
			...
			fs.write(data_write_to_file_root_path + data_write_to_file_sub_path
					+ data_file_prefix_name + (current_grab_page_number - 1)
					+ data_file_suffix_name, page.content);
		}
		// 处理翻页
		page.evaluate(function(current_grab_page_number_copy) {
			...
			var arr = document.querySelectorAll('div#page>a[style]');
	...
	} else if (page.url == ("http://sh.qihoo.com/")) {
		console.log('search_keyword---' + search_keyword);
		page.evaluate(function(keyword) {
			if (document.getElementById("search-kw")) {
				window.alert("search-kw!");
				document.querySelector('input#search-kw').value = keyword;
			} 
			
			if (document.getElementsByName("f2")) {
				window.alert("form submit");
				document.querySelector('form[name="f2"]').submit();
			} 
...
//			var ev = document.createEvent("MouseEvents");
//			ev.initEvent("click", false, true);
//			document.querySelector('button#su').dispatchEvent(ev);
		}, search_keyword);
		if (is_capture_pic) {
			// page.render(pic_capture_save_root_path +
			// pic_capture_save_sub_path
