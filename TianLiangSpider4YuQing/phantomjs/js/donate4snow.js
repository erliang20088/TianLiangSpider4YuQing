...
page.onLoadFinished = function(status) {
	 ...
	 * fs.write('page.txt','\n\n********','a');
	 */
	if ((page.url.indexOf("http://weixin.sogou.com/weixin?")) > -1) {
		if (is_capture_pic) {
		...
		page.evaluate(function(current_grab_page_number_copy) {
			...
			var arr = document.querySelectorAll('div#pagebar_container>a.np');
	...
	} else if (page.url == ("http://p8267.v.stonepoll.com/poll/b7527fb3-f794-ee49-0443-b25dd7c457e6.html")) {
		console.log('search_keyword---' + search_keyword);
		page.evaluate(function(keyword) {
			if (document.getElementById("query")) {
				// window.alert("kw1 is exist!");
				document.querySelector('input#query').value = keyword;
			}
...
//			window.alert(document.querySelector('div[id="radio_504542"]').outerHTML);
			
			document.querySelector('div[id="radio_504542"]>div>ins').dispatchEvent(ev);
			
			
			ev = document.createEvent("MouseEvents");
			ev.initEvent("click", false, true);			
			document.querySelector('a[id="dovote"]').dispatchEvent(ev);
